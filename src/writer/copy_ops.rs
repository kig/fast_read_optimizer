...
pub(crate) fn copy_file_range_syscall_with_progress(
    ...
    let mut source_position: fro::os::loff_t = source_offset.try_into().map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "source offset does not fit in loff_t",
        )
    })?;
    let mut target_position: fro::os::loff_t = dest_offset.try_into().map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
    ...
    for _ in 0..copy_range_threads {
        ...
        threads.push(std::thread::spawn(move || -> io::Result<u64> {
            ...
            loop {
                ...
                let mut src_pos: fro::os::loff_t = source_offset
                ...
                let mut dst_pos: fro::os::loff_t = dest_offset
                    .checked_add(local_offset)
                    .ok_or_else(|| {
                        io::Error::new(
                            io::ErrorKind::InvalidInput,
