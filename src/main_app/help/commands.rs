...
pub(super) fn command_help(name: &str) -> Option<CommandHelp> {
    match name {
        ...
        "grep" => Some(CommandHelp {
            name: "grep",
            usage: "grep [--auto-lift] [--auto|--no-direct|--direct] [-v] [-o] [-n iterations] [-s] [-c config.json] <pattern> <filename>",
            ...
            notes: &[
                ...
                "-o/--only-matching prints each matching substring while keeping -n/-b/-H/-h/-Z/-m/-q/-c/-l/-L/-s/--line-buffered aligned with GNU grep.",
            ...
            examples: &[
                ...
                (
                    "Print only matching substrings from a file",
                    "fgrep -o needle notes.txt",
                ),
                (
        ...
        "cmp" => Some(CommandHelp {
            ...
            name: "fgrep",
            usage: "fgrep [-n] [-b] [-i] [-x] [-w] [-v] [-z] [-r|-R] [-A NUM] [-B NUM] [-C NUM] [-NUM] [-c|-q|-l|-L] [-H|-h] [-Z] [-s] [-T] [-D ACTION] [-d ACTION] [--label LABEL] [--line-buffered] [-a|-I|-U|--binary-files=TYPE] [-m NUM] [--color[=WHEN]|--colour[=WHEN]] [-e PATTERN | -f FILE]... [-V|--version] [--no-ignore-case] [--auto|--no-direct|--direct] [--report-gbps] [pattern] <file> [file ...]",
            ...
            notes: &[
                ...
                "The in-process path covers literal line selection over explicit files or stdin: -F/--fixed-strings, -n/--line-number, -b/--byte-offset, -x/--line-regexp, -w/--word-regexp, -i/--ignore-case, --no-ignore-case, -A/--after-context, -B/--before-context, -C/--context, -NUM, -c/--count, -q/--quiet/--silent, -l/--files-with-matches, -L/--files-without-match, -H/--with-filename, -h/--no-filename, -Z/--null, -z/--null-data, -m/--max-count, --color[=WHEN]/--colour[=WHEN], -s/--no-messages, -T/--initial-tab, --group-separator/--no-group-separator, -D/--devices, -d/--directories (read/skip/recurse), --label, --line-buffered, -a/--text, -I/--binary-files=without-match, -U/--binary, --binary-files=TYPE, -v/--invert-match, -e/--regexp, -f/--file, -r/--recursive/--directories=recurse, -R/--dereference-recursive, --help, and -V/--version.",
                ...
                "-b/--byte-offset prefixes each selected line with its 0-based line-start byte offset; -Z/--null swaps filename separators/terminators to NUL only in filename-printing modes.",
                "-T/--initial-tab inserts GNU grep-style tab separation before selected line content when filename/line-number/byte-offset prefixes are active; --line-buffered flushes each emitted output line only when requested.",
                "-m/--max-count stops after the requested number of selected lines per input; -s/--no-messages suppresses missing-file, unreadable-input, and binary-match diagnostics while preserving grep's exit status rules.",
                "On this Linux host, -D/--devices=read keeps reading device-like positional inputs (including FIFOs), -D/--devices=skip omits them from the search, -d/--directories=read matches GNU grep's local EISDIR surface, -d/--directories=skip omits directory operands, -d/--directories=recurse / -r / --recursive recursively searches all files in the directory tree (symlinks to directories are not followed during traversal), and -R/--dereference-recursive recursively searches all files following symlinks to both files and directories during traversal (circular symlinks are detected via device+inode and skipped).",
                "--label=LABEL renames stdin only in filename-printing modes; ordinary file operands keep their real path labels.",
                "On this Linux host, default binary handling follows GNU grep's NUL-byte heuristic: plain output surfaces report \"binary file matches\", -a/--text and --binary-files=text force text treatment, -I/--binary-files=without-match suppresses matches for binary inputs, and -U/--binary is accepted as the GNU-compatible no-op binary mode.",
                "-e/--regexp and -f/--file can be repeated; if neither is provided, the first positional argument is the pattern.",
                "-w/--word-regexp is handled as fixed-string whole-word matching over the same literal pipeline.",
                "Regex/search-mode families (-E/-G/-P) stay outside this slice.",
                ...
                "--help shows this message and exits.",
                "-V/--version prints the fro fgrep version string and exits.",
            ],
            examples: &[
        ...
        "find" => Some(CommandHelp {
            ...
            notes: &[
                ...
                "Supported in-process families now cover bounded path walking, GNU symlink policy flags (-P/-H/-L/-follow), bounded readdir-race toggles (-ignore_readdir_race/-noignore_readdir_race), glob/path filters, bounded boolean combinators (-a/-and, !/-not, -o/-or), regex path matching (-regex/-iregex with bounded -regextype values), bounded formatter/listing/file-output actions (-printf/-fprintf/-fprint/-fprint0/-ls/-fls), basic constant predicates (-true/-false), -depth traversal order, per-match exec actions (-exec/-execdir/-ok/-okdir), destructive deletion (-delete), and the current ownership/permission/size/time selector set including -used/-fstype and conditional -context support on SELinux systems.",
                "Parentheses, batched -exec ... +, and GNU debug output (-D) remain intentionally omitted; fro does not expose GNU find's internal debug-output family in this bounded slice.",
                ...
                "Default parallel walks still do not guarantee output ordering, but -depth and action-bearing expressions use the serial walker to preserve evaluation order.",
                "-Olevel is accepted in the GNU option slot before paths and is a bounded no-op in the current evaluator; -P keeps GNU find's default no-follow symlink mode explicit.",
                ...
                "-empty, -size, -links, -inum, -perm, -uid/-user, -gid/-group, -nouser/-nogroup, and -readable/-writable/-executable run in-process.",
        ...
        "tar" => Some(CommandHelp {
            name: "tar",
            usage: "tar [--help|--version] (-c|--create|-t|--list|-x|--extract) (-f|--file <archive.tar[.gz|.tgz|.bz2|.tbz|.tbz2|.xz|.txz|.zst|.zstd]>) [-v|--verbose] [-a|--auto-compress|-z|--gzip|--gunzip|--ungzip|-j|--bzip2|-J|--xz|--zstd] [<source>... | -C|--directory <dir>]",
            summary: "Bounded GNU tar-compatible create/list/extract slice for ustar archives; gzip, bzip2, xz, and zstd compressed archives are supported.",
            notes: &[
                "Supported compatibility slice: create (-c/--create), whole-archive list (-t/--list), and whole-archive extract (-x/--extract) with -f/--file; optional extras in this slice are -v/--verbose, -C/--directory for extract, --auto-compress/-a, gzip (-z/--gzip/--gunzip/--ungzip), bzip2 (-j/--bzip2), xz (-J/--xz), zstd (--zstd), --help, and --version.",
                "Common bounded short forms include -cf, -caf, -czf, -cjf, -cJf, -tf, -tvf, -xf, -xzf, -xjf, and -xJf; fro does not claim broader clustered-short GNU tar compatibility beyond the documented whole-archive slice.",
                "Compressed tar supports gzip via -z/--gzip/--gunzip/--ungzip or .tar.gz/.tgz suffixes, bzip2 via -j/--bzip2 or .tar.bz2/.tbz/.tbz2 suffixes, xz via -J/--xz or .tar.xz/.txz suffixes, and zstd via --zstd or .tar.zst/.tar.zstd/.tzst suffixes. --auto-compress/-a uses the archive suffix to select one of those formats.",
                ...
                "FIXME: Unsupported compression/format/incremental/tape families: --no-auto-compress, --compress, --uncompress, --force-local, --format, --format=gnu, --format=posix, --format=v7, --old-archive, --portability, --posix, --pax-option, --pax-option=keyword, --listed-incremental, --incremental, --level, --check-device, --no-check-device, --one-file-system, --check-links, --sparse, --sparse-version, --hole-detection, --sort, --utc, --full-time, --show-defaults, --show-snapshot-field-ranges, --backup, --suffix, --blocking-factor, --record-size, --multi-volume, --tape-length, --volno-file, --new-volume-script, --info-script, --rmt-command, --rmt-command=, --rsh-command, --rsh-command=, --use-compress-program, --files-from, --force-local, --lzma, --lzip, --lzop, -b, -B, -F, -G, -H, -I, -L, -M, -S, -V, -Z, -g.",
        ...
        "bench-base64-wrapped-decode" => Some(CommandHelp {
            ...
            examples: &[("Run one million wrapped decode iterations", "bench-base64-wrapped-decode -n 1000000")],
        }),
        "bench-mmap-write" => Some(CommandHelp {
            ...
            notes: &[],
            examples: &[("Run the mmap write microbenchmark against an existing file", "bench-mmap-write out.bin")],
        }),
        "bench-write" => Some(CommandHelp {
            ...
            notes: &[],
            examples: &[("Run the plain write microbenchmark against an existing file", "bench-write out.bin")],
        }),
        _ => None,
    }
}
