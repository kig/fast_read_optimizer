    ...
    match name {
        ...
        "find" => Some(CommandHelp {
            ...
            notes: &[
                ...
                "Supported in-process families now cover bounded path walking, GNU symlink policy flags (-P/-H/-L), glob/path filters, bounded boolean combinators (-a/-and, !/-not, -o/-or), regex path matching (-regex/-iregex with bounded -regextype values), bounded formatter/listing actions (-printf/-fprintf/-ls/-fls), basic constant predicates (-true/-false), -depth traversal order, per-match exec actions (-exec/-ok/-okdir), and the current ownership/permission/size/time selector set including -used.",
                "Parentheses, batched -exec ... +, alternate execdir actions, and broader traversal families such as -delete/-follow/-context still remain intentionally omitted and are listed in COMPAT below.",
