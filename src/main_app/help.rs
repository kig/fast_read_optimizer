...
pub(super) fn command_help(name: &str) -> Option<CommandHelp> {
    ...
    match name {
        ...
        "sort" => Some(CommandHelp {
            name: "sort",
            usage: "sort [-c|--check] [-m|--merge] [-n|--numeric-sort] [-r|--reverse] [-u|--unique] [-o FILE|--output=FILE] [--auto|--no-direct|--direct] [--report-gbps] [--] [file ...]",
            summary: "Sort newline-delimited records in locale-independent ascending byte or numeric order.",
            notes: &[
                "This bounded slice implements the default bytewise newline-delimited case plus merge/check, C-locale-style numeric prefix ordering, reverse/unique output, and -o output-file writes.",
                ...
                "Unsupported GNU sort features currently return an error, including general keys, month/version/human modes, zero-terminated records, temp-file controls, and locale collation.",
