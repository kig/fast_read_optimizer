...
pub(super) fn command_help(name: &str) -> Option<CommandHelp> {
    ...
    match name {
        ...
        "sort" => Some(CommandHelp {
            name: "sort",
            usage: "sort [-n|--numeric-sort] [-r|--reverse] [-u|--unique] [-o FILE|--output=FILE] [--auto|--no-direct|--direct] [--report-gbps] [--] [file ...]",
            summary: "Sort newline-delimited records in locale-independent ascending byte or numeric order.",
            notes: &[
                "This bounded slice implements the default bytewise newline-delimited case plus C-locale-style numeric prefix ordering, reverse/unique output, and -o output-file writes.",
