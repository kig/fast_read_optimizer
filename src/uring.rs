...
mod stub {
    ...
    impl IoUring {
                ...
                let (user_data, res) = match sqe.op {
                    Some(SqeOp::Read {
                        ...
                        if buf.is_null() {
                            ...
                            let ret =
                                libc::pread(fd, buf as *mut c_void, len, offset as libc::off_t);
                            if ret < 0 {
                                (sqe.user_data, -1)
                            } else {
                    ...
                    Some(SqeOp::Write {
                        ...
                        if buf.is_null() {
                            ...
                            let ret =
                                libc::pwrite(fd, buf as *const c_void, len, offset as libc::off_t);
                            if ret < 0 {
                                (sqe.user_data, -1)
                            } else {
                                (sqe.user_data, ret as isize)
                        ...
                        let res = 'splice_block: {
                            ...
                            while remaining > 0 {
                                ...
                                while write_ptr < nread as usize {
                                    let nw = libc::write(
                                        out_fd,
                                        buf[write_ptr..nread as usize].as_ptr()
        ...
        pub fn peek_for_cqe(&mut self) -> Option<Cqe> {
            let mut guard = self.inner.inner.lock().unwrap();
            guard.cqes.pop_front().map(|c| Cqe {
                user_data: c.user_data,
                res: c.res,
            })
        }
