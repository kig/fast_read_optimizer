...
fn fixed_size_offset_writer_zero_fills_unwritten_gaps() {
    ...
    let writer = ParallelWriter::fixed_size(&cfg, "write", &path, IOMode::PageCache, 16).unwrap();
    writer.write_at_offset(0, b"abcd".to_vec()).unwrap();
...
#[test]
fn transform_io_pairing_classification_matches_expected_matrix() {
    use super::transform::{classify_transform_io_pairing, TransformIoPairingKind};
