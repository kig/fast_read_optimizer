...
pub fn allocate_pipe_output_buffer(min_capacity: usize) -> Vec<u8> {
    ...
    unsafe {
        let _ret = libc::madvise(ptr as *mut libc::c_void, capacity, fro::os::MADV_HUGEPAGE);
    }
...
impl ParallelWriter {
    ...
    pub fn indexed_pipe_with_pool(
        ...
        let join_handle = std::thread::spawn(move || -> std::io::Result<ParallelWriteReport> {
            ...
            // Use fro::os::vmsplice_all for portability (Linux fast-path, write fallback)
            // vmsplice_all defined in src/os.rs
            // The function signature is: pub fn vmsplice_all(pipe_fd: RawFd, buf: &[u8]) -> std::io::Result<usize>
