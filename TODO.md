...
- [x] Make direct-I/O fallback observable and testable so `--direct` users can tell when unsupported filesystems or unaligned tails silently took the page-cache path.
...
- [x] Implement composite signatures for `md` and `dm` stacks.
- [x] Apply device-db profile matching with clear precedence over defaults and under explicit mount overrides.
- [x] Teach `fro-optimize` to write optimized params into `mount_overrides` for the selected mount.
...
- [x] Add deterministic wear/space-based test sizing.
